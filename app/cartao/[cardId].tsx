          ...
          ListHeaderComponent={
            <>
              <View style={gfStyles.headerCard}>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                  <TouchableOpacity onPress={() => router.back()} style={{ padding: 6 }}>
                    <MaterialIcons name="arrow-back" size={22} color={gfColors.text} />
                  </TouchableOpacity>

                  <Text style={[gfStyles.headerTitle, { fontSize: 18 }]}>Cartão</Text>

                  <TouchableOpacity onPress={() => alert("Em breve: configurações do cartão")} style={{ padding: 6 }}>
                    <MaterialIcons name="settings" size={22} color={gfColors.text} />
                  </TouchableOpacity>
                </View>

                <Text style={[gfStyles.headerSub, { marginTop: 10 }]}>
                  {monthLabel(selectedMonth)} • Total do mês: {formatBRL(totalMes)}
                </Text>

                <View style={{ marginTop: 14 }}>
                  <Text style={[gfStyles.sectionTitle, { fontSize: 14 }]}>Meses</Text>
                  <View style={{ marginTop: 10, gap: 10 }}>
                    <View style={gfStyles.chipsRow}>{monthPills.slice(0, 2)}</View>
                    <View style={gfStyles.chipsRow}>{monthPills.slice(2, 4)}</View>
                    <View style={gfStyles.chipsRow}>{monthPills.slice(4, 6)}</View>
                    <View style={gfStyles.chipsRow}>{monthPills.slice(6, 8)}</View>
                  </View>
                </View>
              </View>
              ...
              <View style={gfStyles.sectionHeaderRow}>
                ...
                <TouchableOpacity onPress={() => alert("Em breve: adicionar compra no cartão")}>
            ...
            <View style={gfStyles.card}>
              <View style={gfStyles.cardRow}>
                ...
                <View style={gfStyles.pill}>
                  <Text style={gfStyles.pillText}>{formatBRL(item.valorParcela)}</Text>
                </View>
              </View>

              <Text style={gfStyles.cardSub}>
                {item.subtitle} • Restam {item.parcelasRestantes}/{item.parcelasTotal}
              </Text>
            </View>
          )}
        />
